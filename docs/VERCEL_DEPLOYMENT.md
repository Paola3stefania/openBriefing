...
This guide explains how to deploy OpenBriefing to Vercel with automated daily syncing.
...
OPENBRIEFING_API_KEY=<generated secret>
...
- **Authentication**: `Authorization: Bearer <OPENBRIEFING_API_KEY>` or `x-api-key: <OPENBRIEFING_API_KEY>`
- **Body**:
  ```json
  {
    "tool": "tool_name",
...
- **Example**:
  ...
  curl -X POST https://your-app.vercel.app/api/tools/tool \
    -H "Authorization: Bearer $OPENBRIEFING_API_KEY" \
...
```json
{
  "mcpServers": {
    "openbriefing": {
      "command": "node",
      "args": ["/path/to/openbriefing/dist/index.js"],
      "env": {
...
1. Verify `OPENBRIEFING_API_KEY` is set in environment variables
...
2. **Keep `OPENBRIEFING_API_KEY` secret** - treat it like a password
