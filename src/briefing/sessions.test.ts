...
describe("isSessionAmendable", () => {
  const ORIGINAL_ENV = process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS;

  beforeEach(() => {
    delete process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS;
  });
  ...
  afterEach(() => {
    if (ORIGINAL_ENV === undefined) {
      ...
      process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS = ORIGINAL_ENV;
    }
  });
  ...
  it("honors OPENRUNDOWN_SESSION_AMEND_WINDOW_MS for a custom window", () => {
    const now = new Date("2026-05-06T10:00:00.000Z");
    const fiveMinutesAgo = new Date(now.getTime() - 5 * 60 * 1000);
    const tenMinutesAgo = new Date(now.getTime() - 10 * 60 * 1000);
    process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS = String(7 * 60 * 1000);
  ...
  it("disables soft-end when OPENRUNDOWN_SESSION_AMEND_WINDOW_MS=0", () => {
    process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS = "0";
    const now = new Date("2026-05-06T10:00:00.000Z");
  ...
  it("falls back to the default window when the env var is malformed", () => {
    process.env.OPENRUNDOWN_SESSION_AMEND_WINDOW_MS = "not-a-number";
    const now = new Date("2026-05-06T10:00:00.000Z");
    const oneHourAgo = new Date(now.getTime() - 60 * 60 * 1000);
    expect(isSessionAmendable(oneHourAgo, now)).toBe(true);
  });
