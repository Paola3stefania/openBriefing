...
export class LinearIntegration extends BasePMTool {
  ...
  async createOrGetProject(featureId: string, featureName: string, featureDescription?: string): Promise<string> {
    ...
    const variables = {
      input: {
        ...
        // Associate project with OpenBriefing team to ensure compatibility with issues
  ...
  /**
   * Update project to associate with OpenBriefing team
   * Adds OpenBriefing team to existing teams (doesn't remove existing teams)
   * This makes projects compatible with issues from the OpenBriefing team
   */
  async updateProjectTeam(projectId: string): Promise<boolean> {
    ...
    // Check if OpenBriefing team is already in the project's teams
    if (currentProject.teamIds.includes(this.teamId)) {
      log(`Project ${projectId} already associated with team ${this.teamId}`);
      return true;
    }

    // Add OpenBriefing team to existing teams (don't remove existing teams)
    ...
    try {
      const response = await this.graphqlRequest<{
        ...
        id: projectId,
        input: {
          teamIds: updatedTeamIds, // Add OpenBriefing team to existing teams
        },
      });
  ...
  async createOrGetTeam(teamName: string, teamKey?: string): Promise<string> {
    // If teamKey not provided, generate from teamName (e.g., "OpenBriefing" -> "OPENBRIEFING")
   ...
   */
  async validateTeam(createIfMissing: boolean = false, defaultTeamName: string = "OpenBriefing"): Promise<boolean> {
    if (!this.teamId) {
      if (createIfMissing) {
