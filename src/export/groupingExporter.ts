  ...
  try {
    ...
    if (pmToolConfig.type === "linear") {
      ...
      if (linearTool.validateTeam) {
        await linearTool.validateTeam(true, "OpenBriefing");
              ...
              if (dryRun) {
                ...
                try {
                  ...
                  if (errorMsg.includes("Discrepancy between issue team") && updates.project_id && currentLinearIssue) {
                    ...
                    log(`  Project assignment failed for group ${group.id} (team mismatch: issue belongs to team "${issueTeam}"), attempting to update project to associate with OpenBriefing team...`);
                    
                    // Try to update the project to associate with OpenBriefing team
                    ...
                    if (updateProjectMethod && typeof updateProjectMethod === 'function') {
                      try {
                        ...
                        if (projectUpdated) {
                          log(`  Successfully updated project ${updates.project_id} to associate with OpenBriefing team, retrying group update...`);
            ...
            if (hasChanges) {
              ...
              if (dryRun) {
                ...
                try {
                  ...
                  if (errorMsg.includes("Discrepancy between issue team") && updates.project_id && currentLinearIssue) {
                    const issueTeam = currentLinearIssue.teamName || currentLinearIssue.teamId || "unknown";
                    log(`  Project assignment failed for issue #${issue.issueNumber} (team mismatch: issue belongs to team "${issueTeam}"), attempting to update project to associate with OpenBriefing team...`);
                    ...
                    if (updateProjectMethod && typeof updateProjectMethod === 'function') {
                      try {
                        const projectUpdated = await updateProjectMethod.call(linearTool, updates.project_id);
                        if (projectUpdated) {
                          log(`  Successfully updated project ${updates.project_id} to associate with OpenBriefing team, retrying issue update...`);
                          // Retry the full update now that project is workspace-level
                          try {
                            await updateIssueMethod.call(linearTool, issue.linearIssueId, updates);
                            log(`  Updated issue #${issue.issueNumber} (${issue.linearIssueId}) with changes: ${Object.keys(updates).join(", ")}`);
